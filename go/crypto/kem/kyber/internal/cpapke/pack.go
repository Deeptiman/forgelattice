...
func (p *Kyber) PackPrivateKey(sk *PrivateKey) []byte {
	keyBytes := make([]byte, p.PrivateKeySize)
	offset := 0
	for i := 0; i < p.K; i++ {
		sk.s[i].Pack(keyBytes[offset:])
		offset += PolySize
	}
	return keyBytes
}
...
func (p *Kyber) PackPublicKey(pk *PublicKey) []byte {
	keyBytes := make([]byte, p.PublicKeySize)
	for i := 0; i < p.K; i++ {
		pk.t[i].Pack(keyBytes[PolySize*i:])
	}
	copy(keyBytes[p.K*PolySize:], pk.rho[:])
	return keyBytes
}
